...
# 📊 **Beispiel 3: Autorenverwaltung-Klasse (erweiterte Version)**
print("\n" + "=" * 50)
print("BEISPIEL 3: AUTORENVERWALTUNG-KLASSE")
print("=" * 50)


class AutorenVerwaltung:
    """Klasse zur Verwaltung mehrerer Autoren"""

    def __init__(self):
        self._autoren = []  # Liste für alle Autoren
        print("Autorenverwaltung wurde erstellt")

    def autor_hinzufuegen(self, autor):
        """Fügt einen Autor zur Verwaltung hinzu"""
        self._autoren.append(autor)
        print(f"Autor {autor.vollstaendiger_name()} wurde hinzugefügt")

    def autor_entfernen(self, index):
        """Entfernt einen Autor anhand des Index"""
        if 0 <= index < len(self._autoren):
            autor = self._autoren.pop(index)
            print(f"Autor {autor.vollstaendiger_name()} wurde entfernt")
        else:
            print("Ungültiger Index!")

    def alle_autoren_anzeigen(self):
        """Zeigt alle Autoren an"""
        if not self._autoren:
            print("Keine Autoren in der Verwaltung")
            return

        print(f"\n{'=' * 40}")
        print(f"ALLE AUTOREN ({len(self._autoren)}):")
        print(f"{'=' * 40}")
        for i, autor in enumerate(self._autoren):
            print(f"{i + 1}. {autor.vollstaendiger_name()} - {autor.nationalitaet}")

    def filtern_nach_nationalitaet(self, nationalitaet):
        """Filtert Autoren nach Nationalität"""
        gefiltert = [a for a in self._autoren if a.nationalitaet == nationalitaet]
        print(f"\nAutoren mit Nationalität '{nationalitaet}':")
        for autor in gefiltert:
            print(f"  - {autor.vollstaendiger_name()}")
        return gefiltert

    def filtern_nach_genre(self, genre):
        """Filtert Autoren nach Lieblingsgenre"""
        gefiltert = [a for a in self._autoren if a.lieblingsgenre == genre]
        print(f"\nAutoren mit Genre '{genre}':")
        for autor in gefiltert:
            print(f"  - {autor.vollstaendiger_name()}")
        return gefiltert

    def __del__(self):
        print("Autorenverwaltung wird geschlossen...")


# Test der Autorenverwaltung
verwaltung = AutorenVerwaltung()

# Autoren zur Verwaltung hinzufügen
for autor in autoren_liste:
    verwaltung.autor_hinzufuegen(autor)

# Verschiedene Abfragen
verwaltung.alle_autoren_anzeigen()
verwaltung.filtern_nach_nationalitaet("Deutsch")
verwaltung.filtern_nach_genre("Roman")

# Autor entfernen
print("\n" + "-" * 30)
print("Autor entfernen:")
verwaltung.autor_entfernen(2)  # Entfernt Hesse
verwaltung.alle_autoren_anzeigen()
